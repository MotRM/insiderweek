...
from bokeh.models import (BoxSelectTool, DatetimeTickFormatter, FactorRange,
                          HoverTool, Range1d)
...
def dict_preparation_for_bokeh(unique_name, period_from, period_to, price_name: str):
    ...
    for value in data_db:
        ...
        dict_month_price[f"{month} {year}"] = dict_month_price.get(
            f"{month} {year}", []
        ) + [getattr(value, price_name)]
...
def chart_history_price(data_db):
    ...
    lines.line(
        y_range, data["Цена_1"], color="red", legend_label="Цена 1", line_width=1
    )
    lines.line(
        y_range, data["Цена_2"], color="blue", legend_label="Цена 2", line_width=1
    )
    lines.line(
        y_range, data["Цена_3"], color="orange", legend_label="Цена 3", line_width=1
    )
    lines.line(
        y_range, data["Цена_4"], color="black", legend_label="Цена 4", line_width=1
    )
...
def chart_price_seasonality(unique_name, period_from, period_to):
    ...
    dict_month_price_1 = dict_preparation_for_bokeh(
        unique_name, period_from, period_to, "price_1"
    )
    dict_month_price_2 = dict_preparation_for_bokeh(
        unique_name, period_from, period_to, "price_2"
    )
    dict_month_price_3 = dict_preparation_for_bokeh(
        unique_name, period_from, period_to, "price_3"
    )
    dict_month_price_4 = dict_preparation_for_bokeh(
        unique_name, period_from, period_to, "price_4"
    )
    ...
    lines.line(
        y_range,
        list(dict_month_price_1.values()),
        color="red",
        legend_label="Цена 1",
        line_width=1,
    )
    lines.line(
        y_range,
        list(dict_month_price_2.values()),
        color="blue",
        legend_label="Цена 2",
        line_width=1,
    )
    lines.line(
        y_range,
        list(dict_month_price_3.values()),
        color="orange",
        legend_label="Цена 3",
        line_width=1,
    )
    lines.line(
        y_range,
        list(dict_month_price_4.values()),
        color="black",
        legend_label="Цена 4",
        line_width=1,
    )
